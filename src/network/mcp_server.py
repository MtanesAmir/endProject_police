...
if p2p_client:
    p2p_client.start()
orchestrator = PoliceOrchestrator(p2p_server=p2p_client)
...
def negotiate(message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        cfg = SparConfig(group_id="police_group", group_name="Police Team", seed=123)
        n = message.get("sub_game_number", 1)
        opponent_group = message.get("group_id", "opponent")
        lock_hashes = locks(cfg.scent_model)
        
        mine = our_greeting(cfg, "police", n, f"{n:032x}", lock_hashes, opponent_group)
        if p2p_client:

...
As Team Leader, this development plan outlines the engineering implementation of `police_base_grid_movement` based on the product requirements defined in `police_base_grid_movement_prd.md`.
...
```mermaid
graph TD
    Orchestrator[Police Orchestrator] --> FeatureModule[Police Base Grid & Movement Engine]
    FeatureModule --> DomainState[Domain State / Grid State]
    FeatureModule --> Logger[Audit Logger]
```

### 2. Technical Component Breakdown
- **Component 1**: Define GridPos value class with row, col immutability.
- **Component 2**: Implement MovementEngine class handling direction vectors.
- **Component 3**: Build boundary checking and wall collision validation.
- **Component 4**: Integrate state getters for position queries during turn evaluation.
